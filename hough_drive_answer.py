...
def onMouse(event, x, y, flags, param) :
    ...
    if event == cv2.EVENT_LBUTTONDOWN :
        target_temp.append(x)
    
    elif event == cv2.EVENT_MOUSEMOVE:
        #cv2.putText(frame,"Mouse Point x : %d" % x, (400,100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255))
        #cv2.putText(frame,"Mouse Point y : %d" % y, (400,150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255))
        print('현재 마우스 좌표 : ', x, y)
...
def start():
    ...
    while cap.isOpened():                                               # 실행할 수 있도록 수정
        ...
        if frame_idx==30:
            ...
            key = cv2.waitKey(0) & 0xFF         # 키보드 입력 전 무한 대기
    ...
    #target=pd.DataFrame(np.reshape(target_temp,(-1,4)))

    #target.to_csv('./Line_detection/target.csv',header=False, index=False)
